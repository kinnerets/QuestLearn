...
import { getPendingTaskApprovals, resolveTaskApproval } from '@/lib/db';
import { parentUnlocked } from '@/lib/session';

export const dynamic = 'force-dynamic';

export async function GET() {
  if (!parentUnlocked()) return NextResponse.json({ ok: false, approvals: [] });
  const approvals = await getPendingTaskApprovals();
...
export async function POST(req: Request) {
  ...
  const ok = await resolveTaskApproval(id, action);
